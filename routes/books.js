...
booksRouter.put("/", (req, res) => {
  ...
  const bookId = booksArr.findIndex((oldBook) => oldBook.id === book.id);

  if (bookId === -1) {
    return res.status(400).json({ message: "book not found!" });
  }

  booksArr[bookId] = book;
